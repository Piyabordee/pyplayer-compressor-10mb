...
# Lazy import helper for qthelpers
def _get_qthelpers():
...
def loadConfig(gui, filename: str = constants.CONFIG_PATH) -> ConfigParseBetterQt:
    ...
    # Sync the auto-compress checkbox to match the loaded auto_compress_after_trim setting
    # The checkbox should reflect the config value, not override it
    if hasattr(settings, 'checkAutoCompress'):
        settings.checkAutoCompress.setChecked(gui.auto_compress_after_trim)
    ...
    cfg.setSection('keys')
    for layout in _get_qthelpers().formGetItemsInColumn(settings.formKeys, 1):
        items = tuple(_get_qthelpers().layoutGetItems(layout))
    ...
    logger.debug(f'It took {time.time() - start:.4f} seconds to load this config.\n')
    return cfg
...
def saveConfig(gui, filename: str = None):
    ...
    cfg.setSection('keys')
    for widget in _get_qthelpers().formGetItemsInColumn(gui.dialog_settings.formKeys, 1):
        items = tuple(_get_qthelpers().layoutGetItems(widget))
