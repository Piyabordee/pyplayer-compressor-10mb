...
# Lazy import helper for qtstart.exit
def _exit_app(gui):
...
def download_update(self, latest_version: str, download_url: str, download_path: str) -> None:
    ...
    try:
        ...
        import shutil
        active_updater_path = _get_unique_path()(os.path.join(constants.CWD, 'updater_active') + ext)
        ...
        _get_qthelpers().getPopup(title='Update download failed',                  # download_update does not occur inside a thread, so this is safe
                           icon=QMessageBox.Warning,
...
def update_migration(self, old_version: str) -> None:
    ...
    if older_than('0.3.0'):     # updated from 0.1.0 - 0.2.0
        _get_qthelpers().getPopup(title='Update successful, but reinstall recommended',
